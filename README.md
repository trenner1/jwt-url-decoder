...
real person's claims, or an internal session — into a text box on someone
else's server.
...
JWT. Nothing opens on its own and nothing is announced — the badge just goes
on, per tab, and goes off again when you navigate somewhere boring.
...
`id_token` and an `access_token` — you get a list first, each pill
colour-coded by expiry, and you drill into whichever one you care about. One
token skips the list entirely, because a list of one is a rude thing to do to
somebody.
...
every tab's URL — `activeTab` only grants access after you click, which would
...
is `clipboardWrite` — the copy buttons ride the user gesture of their own
click, so the permission isn't needed.

## Install

Not on the Web Store. Build it and load it unpacked:
...
unpacked**, and select the repository root — the folder with `manifest.json`
in it, not `dist/`.

Requires Chrome 116 or later, which is when `sidePanel.open()` arrived.
...
Open that URL and you should get a badge, then a two-item list with one green
pill and one red one.
...
After changing anything, rebuild and hit the reload icon on the extension card.
Editing source alone does nothing — Chrome runs the bundles.
