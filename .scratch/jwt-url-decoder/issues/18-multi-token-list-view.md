...
- [ ] When `scanUrlForJwts` returns more than one token for the active
...
- [ ] Clicking a pill navigates to that token's full detail view from
      ticket 17.
- [ ] When exactly one token is found, the panel skips the list and shows
...
- [ ] Verified manually on a URL with two tokens of different `status`
